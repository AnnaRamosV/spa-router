<template>
<div class="h-screen flex flex-col">
    <header class="flex items-center h-14 sm:h-16 px-4 md:px-6 lg:px-8 border-b border-gray-300">
      <div>
        <a href="#">
          <!--<img src="@/assets/logo.svg" width="40" height="40" alt="">-->
        </a>
      </div>

      <nav class="ml-auto flex flex.col items-end">

        <div class="space-x-4">
        <RouterLink :to=" {name: 'home'}" exact-active-class="underline">Home</RouterLink>
        <RouterLink to="features" exact-active-class="underline">Features</RouterLink>
        <RouterLink to="pricing" exact-active-class="underline">Pricing</RouterLink>
        <RouterLink to="contact" exact-active-class="underline">Contact</RouterLink>
      </div>

      <div class="space-x-4">
        <RouterLink to="/pokemon/1">Pokèmon</RouterLink>
        <RouterLink to="/auth">Login</RouterLink>
      </div>
      </nav>

    </header>

    <main class="bg-slate-100 flex-1 flex items-center justify-center py-6">
    <router-view v-slot="{Component}">
      <keep-alive>
        <component :is="Component"/>
      </keep-alive>
    </router-view>
    </main>

    <footer class="flex items-center h-14 px-4 border-t border-gray-300 sm:h-16 md:px-6 lg:px-8">
      <p class="flex-1 text-sm text-gray-500 text-center">
        @ {{new Date().getFullYear()}}Acme Corporation. Derechos reservados
      </p>

    </footer>
</div>
</template>